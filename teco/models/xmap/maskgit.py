...
class MLPShard(nn.Module):
    ...
    def __call__(self, x):
        x = f_psum(x)
        print(x.shape, self.hidden_dim * 8)
        x = nn.Dense(self.hidden_dim * 8 // self.num_shards, dtype=self.dtype)(x)
        x = nn.gelu(x)
        x = nn.Dense(self.hidden_dim * 4, dtype=self.dtype)(x)
        x = nn.gelu(x)
        x = nn.Dense(self.hidden_dim * 4, dtype=self.dtype)(x)
        x = nn.gelu(x)
        x = nn.Dense(self.hidden_dim * 4, dtype=self.dtype)(x)
        ...
        x = nn.Dense(self.hidden_dim * 4, dtype=self.dtype)(x)
        x = nn.gelu(x)
        x = nn.Dense(self.hidden_dim * 2, dtype=self.dtype)(x)
        x = nn.gelu(x)
    ...
    def model_spec():
        return sharding.GenericDict({
            ...
            'Dense_4': sharding.Dense(use_bias=True, axis=1),
            'Dense_5': sharding.Dense(use_bias=True, axis=1),
            'Dense_6': sharding.Dense(use_bias=True, axis=1),
            'Dense_7': sharding.Dense(use_bias=True, axis=1),
            'Dense_8': sharding.Dense(use_bias=True, axis=1),
            'Dense_9': sharding.Dense(use_bias=True, axis=1),
        })

class MaskGitAblationShard(nn.Module):
    num_shards: int
    shape: Tuple[int]
    vocab_size: int
    vocab_dim: int
    ...
    def setup(self):
        ...
        self.net = MLPShard(self.num_shards, self.vocab_dim, self.dtype)
